...
# Load the model
print('Loading model...')
model = seggradcam.UNetWithHooks()
...
sgc = seggradcam.SegGradCAM(setup=setup, model=model)

print('\nPlotting Segmentation-Grad-CAM...')
# iterate over nodemap input samples
...
# setup separate SegGradCAM-class objects for all layers in 'feature_names'
seg_grad_cams = {}
for name in VISU_LAYERS:
    seg_grad_cams[name] = seggradcam.SegGradCAM(setup, model, name)
